...
def convert_to_dataframe(data):
    ...
    df['Timestamp'] = pd.to_datetime(df['Timestamp'])  # 첫 번째 열을 시간 형식으로 변환
    df.set_index('Timestamp', inplace=True)  # 시간 열을 인덱스로 설정
    print(df)

    # null 데이터 제거 (NaN 값이 포함된 행 제거)
    df.dropna(how="all", inplace=True)  # 모든 값이 NaN인 행 제거

    # 열 이름 확인을 위해서 출력 (필요에 따라 제거 가능)
    # print("변환된 데이터프레임:", df.columns)
    return df
